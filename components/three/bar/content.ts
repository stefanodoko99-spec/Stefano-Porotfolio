...
/**
 * Where the written portfolio lives, and what the contact sign opens.
 *
 * Relative, not absolute. This pointed at the deployed origin, which meant a
 * preview build or a local run sent the visitor to production to read the
 * about page. The scene is now the front door of that same site, so the form
 * is a path away rather than a domain away — and `/info` rather than `/` is
 * where the sections moved when the diorama took the homepage.
 */
const SITE = ''
...
export const CONTENT = {
  ...
  contactUrl: `${SITE}/en/info#contact`,
  ...
  social: [
    { label: 'Contact form', url: `${SITE}/en/info#contact` },
