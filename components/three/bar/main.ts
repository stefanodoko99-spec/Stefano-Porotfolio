...
/**
 * Start the shop inside an already-rendered container.
 *
 * The Vite build ran this on import and reached for `#app` itself. Inside
 * React the component owns the DOM, so this is called on mount and hands back
 * a teardown: a scene that keeps its animation frame alive after unmount holds
 * the whole GPU context and every texture with it.
 */
