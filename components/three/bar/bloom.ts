...
export function createBloom(renderer: WebGLRenderer, scene: Scene, camera: Camera, divisor: number, strength = 1.3, smaa = false): Bloom {
  ...
  // SMAAPass took its size in the constructor up to three 0.17x and takes none
  // now — the composer sets it on the pass when it sizes the chain.
  if (smaa) finalComposer.addPass(new SMAAPass())
